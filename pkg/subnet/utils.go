...
func splitUnallocatedIPs(s v1alpha1.PoolIPUnallocation, n int) []v1alpha1.PoolIPUnallocation {
	result := []v1alpha1.PoolIPUnallocation{}
	if n == 0 {
	...
	sliceLength := len(s)
	subSliceLength := (sliceLength + n - 1) / n // 每份切片的长度
	for i := 0; i < sliceLength; i += subSliceLength {
		end := i + subSliceLength
		if end > sliceLength {
			end = sliceLength
		}
